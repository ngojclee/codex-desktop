#!/usr/bin/env python3
"""Patch P - expose Max reasoning effort for gpt-5.6-sol.

The model catalog can advertise `max` for gpt-5.6-sol, but the renderer has
two additional filters:

1. The Work power slider has a static Sol sequence ending at `ultra` and omits
   a `gpt-5.6-sol:max` entry.
2. The model-list filter removes efforts absent from the persisted
   `enabledReasoningEfforts` set. Existing profiles may have been created
   before `max` was enabled.

This patch adds Sol Max to the power sequence and preserves catalog-supported
`max` efforts even when an older persisted allowlist omits them.
"""
import argparse
import json
import re
...
def patch_filter(text: str):
    ...
    if match is None:
        if re.search(r"\.has\([A-Za-z_$][A-Za-z0-9_$]*\)\|\|[A-Za-z_$][A-Za-z0-9_$]*===`max`", text):
            return text, False
        raise RuntimeError("Could not find the enabledReasoningEfforts model-list filter")
    ...
    replacement = (
        ...
        f"({enabled}.has({effort})||{effort}===`max`){FILTER_MARKER})"
    )
    return text[: match.start()] + replacement + text[match.end() :], True
...
def find_targets(asar: Path):
    ...
    for path, meta in walk(header):
        ...
        elif "model-list-filter-" in path and "enabledReasoningEfforts" in text:
            targets.append((path, meta, text, "filter"))
...
def verify(asar: Path):
    ...
    if not filter_paths:
        raise SystemExit("Verification failed: catalog-supported max effort is still filtered")
    return sorted(set(power_paths)), sorted(set(filter_paths))
...
def main():
    ...
    if kinds != {"power", "filter"}:
        raise SystemExit(f"Could not find both Patch P targets; found: {sorted(kinds)}")
    ...
    for path, _meta, text, kind in targets:
        scanned.append(path)
        try:
            patched_text, changed = patch_power(text) if kind == "power" else patch_filter(text)
    ...
    power_paths, filter_paths = verify(asar)
    print(
        json.dumps(
            {
