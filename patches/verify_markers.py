...
def main():
    ...
    checks = (
        ...
        ("Patch C v2 — guard `if(!this.fetchedRecentConversations)`", lambda: "if(!this.fetchedRecentConversations)" in txt, True),
        ("Patch C — `__cap=2000` marker", lambda: "__cap=2000" in txt, True),
